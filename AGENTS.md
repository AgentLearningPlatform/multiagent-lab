...
- **待领取（已排期 M24，PoC 先行）**：REQ-156 DeepSeek Harness 推理后端适配器（M13 cliAdapter 第 4 例；dsh headless 契约 PoC 为 go/no-go 门；Minimal 裁剪+锁版本）——research/DeepSeek_Harness接入可行性_20260925.md。

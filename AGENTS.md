...
- **待领取（立项待开发）**：~~REQ-148~~（✅ a64178f）/ ~~REQ-149 展示级别作用于历史回放~~（✅ f03c1fe+5ee3ddd，2026-09-25——levelGated 历史与回放过滤 + debug_persist UI 面与联动 + 诚实标注 + 顺修 M17 model.step SSE 双发 bug；01 v0.40/02 v0.57/20 v1.8）；REQ-150 对比界面系统优化（19e/f 基座体验重构——知识库/本体/技能选项迁各窗格头、继承/覆盖状态显性化、窗格视觉系统化；建议与 M19 的 REQ-143/144 合并实施）——2026-09-25 主人提出。

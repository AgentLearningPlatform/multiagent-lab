...
- 智能体侧：M12 全项收尾（02 v0.34 模型预设 + v0.35 Git 深度版）；项目绑定本地目录 bugfix 交付（02 v0.33）；**REQ-14(恢复) ask_human 中断恢复已交付（02 v0.38）；REQ-14(恢复②) 工具调用人工审批已交付（02 v0.39，LG-8 危险操作审批，tool_approval=all；checkpoint 进程内，重启失效）**。
- 知识库侧：**M16 知识图谱增强已立项排期**（REQ-127~130：图谱浏览/多跳检索/抽取治理/社区摘要全局问答；11 v0.6 + 12 v0.9 §3.5 + 02 §12 M16）——阶段一 P1 待开发。

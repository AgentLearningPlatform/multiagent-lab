...
- 知识库侧：**M16 知识图谱增强已立项排期**（REQ-127~130：图谱浏览/多跳检索/抽取治理/社区摘要全局问答；11 v0.6 + 12 v0.9 §3.5 + 02 §12 M16）——**阶段一（REQ-127/128）已交付（02 v0.41）；阶段二 REQ-129 抽取治理已交付（02 v0.42：审核队列/实体合并/质量面板/库级抽取覆写）**；REQ-130 社区摘要与全局问答随触发条件评估。

...
> 版本 v1.1 ｜ 2026-09-26 ｜ 立项：REQ-171（✅ 已立项——2026-09-26 主人拍板 §11 四项决策点全部接受；P1 先行段挂 M-O14，开发排期待定）
...
> 全文见 [../../research/本体构建理论论文线综述_20260926.md](../../research/本体构建理论论文线综述_20260926.md)（35 篇：方向一 LLM 驱动构建 11 篇/方向二本体学习经典理论 8 篇/方向三交互式对话式构建 7 篇/方向四多源异构途径 9 篇；高置信标识约 20 篇含 DOI/arXiv 直验，检索局限已在文中诚实标注）。

**四条核心量化证据**：

| 证据 | 数字/结论 | 论文 | 对本方案的支撑 |
|---|---|---|---|
| 数据驱动 schema 归纳可行性 | AutoSchemaKG 免预定义 schema 归纳，与人工 schema 语义对齐 92%（5000 万文档→9 亿+节点） | arXiv 2505.23628 | 路径⑤ KB 构建的「数据驱动归纳」模式可行性论证 |
| schema 约束抑制幻觉 | Wikidata-schema 约束引导 LLM 抽取显著抑制幻觉、提升一致性（与 AutoSchemaKG 形成双模式对照） | arXiv 2412.20942 | **双模式设计依据**：有成熟领域本体走 schema 约束抽取，冷启动走归纳+人工收敛 |
| 人工在环是理论共识 | X-HCOME+/驾驶域蒸馏/IDEA2 三条独立证据链一致：人工早期/持续介入产出质量显著优于全自动 | 2512.14288/2307.11769/2604.01344 | 候选→人工确认→生效三态流；介入点应前移到生成过程中 |
| CQ 跨领域泛化 | CQ+用户故事→本体生成跨 6 领域 95 条 CQ 表现一致 | arXiv 2504.17402 | ontochat CQ 引导路径的可靠性背书 |

**七条可操作启示（与本方案设计对接）**：

1. **schema 先行 vs 数据驱动双模式**（启示 1）→ 六路径天然覆盖：①②④ 为 schema 约束侧，⑤ 为数据驱动侧（归纳产物作候选本体、人工确认后入库——正是 REQ-108 草稿必审设计）；
2. **人工在环前移**（启示 2）→ 自修复循环 3 轮未过转人工确认队列（§8 风险 4），非仅事后审校；
3. **CQ 一等公民**（启示 3）→ CQ 引出→验证→驱动生成→**CQ 驱动 SPARQL 评测**（可移植到 facade 的 sparql_query，P2 ontochat 深化项）；
4. **评测复用 LLMs4OL 四任务分解 + OAEI-LLM 幻觉三通道**（启示 4）→ qualitygate 质量分维度设计参照，幻觉评测单列（正确/幻觉/无趣），不自造评测体系；
5. **混合管线 > 单点 LLM**（启示 5）→ 抽取引擎可插拔组合（嵌入检索+LLM 抽取+规则后验）——KB 路径策略 C 深化时遵循；
6. **增量消歧而非一次性重建**（启示 6）→ 多途径汇入维护全局实体登记与消歧队列（P2 导入审查底座的合并策略延伸）；
7. **可解释可审计**（启示 7）→ 构建过程产出结构化决策痕迹，与既有 PROV-O 审计线天然契合；学习目标表达力分级默认轻量 RDFS/OWL-RL 起步。

**理论线与开源实践线的交叉验证**：两条独立证据链（业界工具实现 vs 学术评测）收敛到同一结论——**生成质量的上限由验证反馈的质量决定**（实践线：F1 0.717 工具链闭环 vs 0.323 直读；理论线：人工在环三证据链 + 幻觉需单列评测）。这是本方案把「质量门禁」设为公共底座而非单路径增强的根本依据。
...
**本方案立项**：REQ-171「本体多途径构建平台可落地方案」——✅ **已立项**（2026-09-26 主人拍板 §11 四项决策点全部接受），登记于 [../../docs/18_REQ编号注册表.md](../../docs/18_REQ编号注册表.md) v1.30 与 [../../docs/03_本体_需求文档.md](../../docs/03_本体_需求文档.md) v0.33 §2.14；P1 先行段挂 M-O14，开发排期待定。
...
## 11. 决策点拍板记录（2026-09-26 主人拍板：四项全部接受）

1. **P1 三个子项整体立项** ✅——质量门禁（qualitygate 包）+ LLM 自修复循环（llmcreate 升级）+ LOV 词表搜索薄层，挂 M-O14 段，开发排期待定；
2. **质量门禁检查项首批清单** ✅——按 §8-7 建议 8~12 项范围确认，具体清单开发时数据驱动配置（JSON），按误报率迭代；
3. **路径⑥ ROBOT Template 纳入 P2 评估** ✅——CSV 批量建模场景确认后决定是否实现；
4. **里程碑编号启用 M-O14 段** ✅——P1 先行段挂 M-O14；子需求拆分时按序分配新 REQ 号（见 §10 建议清单）。

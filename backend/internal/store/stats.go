...
func (s *Store) loadRunStarted() (map[string]runStartedMeta, error) {
	rows, err := s.DB.Query(`SELECT run_id, conversation_id, data FROM run_event WHERE type = 'run.started'`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) loadRunFinished() (map[string]runUsage, error) {
	rows, err := s.DB.Query(`SELECT run_id, data FROM run_event WHERE type = 'run.finished'`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
